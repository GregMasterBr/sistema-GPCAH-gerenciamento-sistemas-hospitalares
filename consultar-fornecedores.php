...
<?php 
  ...
  include ('includes/connection.php');
    ...
    $queryUsers='SELECT * FROM fornecedores order by created_at desc';
 				    ...
 				    <div class="col-md-12">
					   <div class="bs-example1" data-example-id="contextual-table">
					    <table class="table">
			              ...
			              <tfoot>
			               <tr>
			               		<td colspan="7" >Fornecedores encontrados: <?php echo $nregistros;  ?> </td>    
			                </tr>
			              </tfoot>					      
					      <tbody>
			               <?php
			                  for ($i=0; $i<$nregistros; $i++){
			                    $registro = mysqli_fetch_assoc($result);
                          	     $cadastro ='editar-fornecedores.php?id='.$registro['id'];


			                      echo'<tr>';
			                          echo'<td class="text-capitalize" id='.$registro['id'].'>'.($i+1) .'</td>';
			                          echo'<td class="text-capitalize">'.($registro['empresa']).'</td>';
